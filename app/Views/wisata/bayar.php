...
<html lang="en">
        ...
        <section class="page-section bg-light" id="destinasi">
            <div class="container">
                <div class="text-center">
                    <h2 class="section-heading text-uppercase">Daftar pembayaran tiket anda</h2>
                </div>
                <div class="row">
  ...
  <tr>
      ...
      <td><?= $row->total;?></td>
      <td>  <a href="https://app.sandbox.midtrans.com/snap/v2/vtweb/<?=$row->snap;?>" class="btn btn-outline-success" target="_blank">Bayar</td>
      <td>
        <?php
        if($row->status == 200){
            $ket="Sudah Bayar";
      ...
      <td>
        <?php
        if($row->status != 200){?>
        <a href="<?= base_url('wisata/setatus/'.$row->id_pesan);?>" class="btn btn-info">Klik jika sudah bayar</a>
        <?php
        }
        ?>
        </td>
